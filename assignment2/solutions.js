 ...
 *  - concatenate the first name and last name and assign to a `name` variable
 *  - call console.log() with the `name` variable
 *  - return a new Array with all manager users.
 ******************************************************************************/
...
userUtils.getManagers = function() {
  ...
  for (let x of users) {
    ...
    if (x.isManager === true) {
      managers.push(x);
      this.name = x.name.first + ' ' + x.name.last;
...
userUtils.getOldestUser = function() {
  ...
  users.forEach(function(x) {
    ...
    let age = getAgeInYears(new Date(x.birthDate));
    if (age > oldest) {
      oldest = age;
      id = x.id - 1;
    }
...
userUtils.getProfiles = function(width, height) {
  ...
  userUtils.getProfiles = function(width, height) {
    ...
    let map1 = users.map(function(x) {
      ...
      let profile = {
        id: x.id,
        email: x.name.first + ' ' + x.name.last + ' ' + '<' + x.contact.email + '>',
        image: 'https://faces.com/user/' + x.id + '?width=' + width + '&height=' + height
...
userUtils.updateAccessCount = function(amount, ids) {
  ...
  for (let i = 0; i < idArray.length; i++) {
    users.find(function(x) {
      if (x.id == idArray[i]) {
        x.accessCount += amount;
        let info = {
          id: idArray[i],
          accessCount: x.accessCount
        };
        accessCountArray.push(info);
      }
    });
  }
...
userUtils.search.byName = function(name, fuzzy) {
  ...
  if (fuzzy == true) {
    ...
    Match = users.filter(function(x) {
      return (
        x.name.first.toUpperCase().startsWith(name.toUpperCase()) ||
        x.name.last.toUpperCase().startsWith(name.toUpperCase())
      );
    });
  } else {
    Match = users.filter(function(x) {
      return x.name.first == name || x.name.last == name;
    });
  }
  return Match;
};
  ...
  for (let i = 0; i < countries.length; i++) {
    valid = valid.concat(
      users.filter(function(user) {
        return countries[i] == user.address.country;
      })
    );
  }
  return valid;
...
userUtils.createWelcomeEmails = function() {
  ...
  newUser = users.filter(function(x) {
    return x.accessCount == 0;
  });

  for (var i = 0; i < newUser.length; i++) {
    result[newUser[i].contact.email] = 
    `Dear ${newUser[i].name.first} ${newUser[i].name.last}
Welcome to imaginary application!  We're so happy you joined, and wanted
to take a moment to say hello. Our other users from ${newUser[i].address.city}, ${newUser[i].address.country} will
be glad to have you join them.
